...
function preload(){
  ...
  prizeimg = loadImage("images/prize.png");
...
function draw() {
  ...
  if (World.frameCount % 150 == 0) {
   ...
   if(pinkCG.isTouching(mainCyclist)){
    ...
    if(mainCyclist.isTouching(prizeGroup)){
      prize.x = 600;
      prize.y = 150;
      gameState = END;
      prize.velocityY = 0;
      prize.velocityX = 0;
      prize.addImage("won",wonimg);
      prize.changeImage("won");
      prize.scale=1.5;
    }
...
function winner(){
  prize = createSprite(1100,Math.round(random(50,250)));
  prize.scale=0.06;
  prize.velocityX = -(6 + 2*distance/150);
  prize.addAnimation('prizeimg',prizeimg);
  prize.setLifetime=170;
